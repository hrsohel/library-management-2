...
const page = async () => {
     ...
     return (
          <>
               <div className='w-full py-4 h-[90vh] rounded-md bg-slate-300 md:gap-3 gap-1 overflow-y-scroll flex items-stretch justify-center flex-wrap'>
                    {
                         bookdata && bookdata.map((value: bookDataType) => <div key={value._id} className='bg-white md:p-3 p-1 relative rounded-md w-[10rem] md:w-[15rem] md:hover:-translate-y-3 duration-500 '>
                              ...
                              <Link className='px-2 py-0.5 mx-auto bottom-0 left-0 mt-2 block text-center rounded-full md:text-lg text-sm text-white bg-[#0a298c]' href={`/about-book/${value._id}`}>View details</Link>
                         </div>)
                    }
